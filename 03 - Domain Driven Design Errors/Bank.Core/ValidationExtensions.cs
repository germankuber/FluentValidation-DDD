...
namespace Bank.Core
{
    ...
    {
        public static IRuleBuilderOptions<T, TProperty> NotNullEmpty<T, TProperty>(this IRuleBuilder<T, TProperty> ruleBuilder,
            string message) =>
            ruleBuilder
                .NotNull()
                .NotEmpty()
        ...
        public static Result<T> ValidateWithResult<T>(this AbstractValidator<T> validator, T entity)
            => ValidateResult(entity, validator.Validate(entity));
